...
RSpec.describe ZOTP::TOTP do
  let(:now)   { TEST_TIME }
  let(:token) { TEST_TOKEN }
  let(:totp)  { ZOTP::TOTP.new 'JBSWY3DPEHPK3PXP' }
  ...
  describe '#at' do
    ...
    context 'RFC compatibility' do
      let(:totp) { ZOTP::TOTP.new('GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ') }
  ...
  describe '#verify' do
    ...
    context 'RFC compatibility' do
      let(:totp) { ZOTP::TOTP.new 'wrn3pqx5uqxqvnqr' }
  ...
  describe '#now' do
    ...
    context 'Dropbox 26 char secret output' do
      let(:totp) { ZOTP::TOTP.new 'tjtpqea6a42l56g5eym73go2oa' }
