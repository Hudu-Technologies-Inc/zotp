  ...
  class CLI
    ...
    def errors
      if %i[time hmac].include?(options.mode)
        ...
        elsif options.secret.to_s.chars.any? { |c| ZOTP::Base32::CHARS.index(c.upcase).nil? }
    ...
    def output
      ...
      if options.mode == :time
        ZOTP::TOTP.new(options.secret, options).now
      elsif options.mode == :hmac
        ZOTP::HOTP.new(options.secret, options).at options.counter
      end
    end

    def arguments
      @arguments ||= ZOTP::Arguments.new(filename, argv)
    end
